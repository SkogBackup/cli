...
## Keep notes
Create a markdown file under the notes/features/ folder for the feature (create folder if needed). If you are creating a feature branch, use the same name.
...
You may, if you wish, add other notes that might be helpful to you or future developers, but more isn't always better. Be brief and helpful.

## Understand the feature
1. First read the README.md and any relevant docs it points to.
2. Run existing tests to verify current behavior.
3. Ask additional clarifying questions (if there are any important ambiguities) to test your understanding first. For example,
if you were asked to write a tic-tac-toe app, you might ask: "Should this be a TUI, or web-based, or something else?"
4. Update the README.md as needed to reflect insights gained or new information that would be relevant to you in the future or
...
run all tests & commit to git (only when all tests are passing) but don't push
...
run all tests & commit to git (only when all tests are passing)

Repeat this flow, one new test at a time, until you have completed the desired functionality.

**When to stop**: Feature is complete when all acceptance criteria have passing tests.
...
Implements login/logout with JWT tokens.
Adds password validation and error handling.
