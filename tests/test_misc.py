...
class TestScriptFunctions:
    ...
    def setup_method(self):
        ...
        subprocess.run([
            "uv", "run", "skogcli", "config", "set", 
            "script.user_scripts_dir", str(self.test_scripts_dir)
        ], check=True)
        subprocess.run([
            "uv", "run", "skogcli", "config", "set",
            "script.metadata_dir", str(self.test_metadata_dir)
        ], check=True)
    ...
    def teardown_method(self):
        ...
        subprocess.run([
            "uv", "run", "skogcli", "config", "set",
            "script.user_scripts_dir", "/home/skogix/skogai/config/scripts"
        ], check=True)
        subprocess.run([
            "uv", "run", "skogcli", "config", "set",
            "script.metadata_dir", "/home/skogix/skogai/config"
        ], check=True)
