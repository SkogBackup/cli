...
script_app = typer.Typer(help="Script management commands", no_args_is_help=True)
    ...
    if not template_path.exists():
        ...
        if not template_path.exists():
            raise FileNotFoundError(
                f"Template {template_name} for {script_type} not found"
            )
...
def get_user_scripts_dir() -> Path:
    ...
    # Crash and burn
    raise RuntimeError(
        "No user scripts directory configured. Set 'script.user_scripts_dir' config or SKOGAI_SCRIPTS_DIR environment variable."
    )
...
def list_scripts(
    ...
    show_metadata: bool = typer.Option(
        False, "--metadata", "-m", help="Show script metadata"
    ),
):
    ...
    if not scripts:
        ...
        console.print(
            f"Add scripts to [bold]{get_user_scripts_dir()}[/] to get started."
        )
        return
    ...
    if show_metadata:
        ...
        for script in scripts:
            ...
            location = (
                "User" if str(get_user_scripts_dir()) in str(script) else "Global"
            )

            table.add_row(script.stem, script_type, location, last_updated, description)
        ...
        for script in scripts:
            ...
            location = (
                "[cyan]user[/]"
                if str(get_user_scripts_dir()) in str(script)
                else "[magenta]global[/]"
            )

            console.print(
                f"  [bold]{script.stem}[/] - {script_type} script ({location})"
            )
...
def run_script(
    ...
    update_script_metadata(
        script_path, {"run_count": run_count, "last_run": datetime.now().isoformat()}
    )
    ...
    if script_path.suffix == ".py":
        ...
        try:
            ...
            if spec is None or spec.loader is None:
                console.print(
                    f"[bold red]Error:[/] Failed to load Python script '{name}'."
                )
                return
            ...
            if hasattr(module, "main"):
                ...
                console.print(
                    "[yellow]Warning:[/] Python script has no 'main' function."
                )
...
def create_script(
    ...
    global_script: bool = typer.Option(
        False, "--global", "-g", help="Install as a global script"
    ),
    description: str = typer.Option(
        "", "--description", "-d", help="Description of the script"
    ),
    ...
    if global_script:
        ...
        if not os.access(global_dir.parent, os.W_OK):
            console.print(
                "[bold red]Error:[/] You don't have permission to create global scripts."
            )
    ...
    try:
        ...
        console.print(
            f"[yellow]Warning:[/] Template '{template}' not found for {type}."
        )
        if available_templates:
            console.print(
                f"Available {type} templates: {', '.join(available_templates)}"
            )
    ...
    if edit:
        ...
        try:
            ...
            console.print(
                f"[bold red]Error:[/] Editor '{editor_cmd}' not found. Set the EDITOR environment variable or use --editor."
            )
        except Exception as e:
            console.print(f"[bold red]Error:[/] {str(e)}")
...
def edit_script(
    ...
    if not os.access(script_path, os.W_OK):
        console.print(
            "[bold red]Error:[/] You don't have permission to edit this script."
        )
    ...
    try:
        ...
        if exit_code == 0:
            # Update metadata
            update_script_metadata(
                script_path, {"last_edited": datetime.now().isoformat()}
            )
            console.print("[green]Script edited successfully.[/]")
        else:
...
def remove_script(
    ...
    if not os.access(script_path, os.W_OK):
        console.print(
            "[bold red]Error:[/] You don't have permission to remove this script."
        )
        console.print("Try running with sudo if it's a global script.")
        return
    ...
    if not force:
        location = (
            "global" if str(get_global_scripts_dir()) in str(script_path) else "user"
        )
        confirm = typer.confirm(
            f"Are you sure you want to remove {location} script '{name}'?"
        )
        if not confirm:
            return
...
def script_code(
    ...
    output_file: Optional[Path] = typer.Option(
        ...
        "--output",
        "-o",
        help="Write content to this file instead of updating the script",
    ),
    global_script: bool = typer.Option(
        True, "--global/--no-global", help="Include global scripts"
    ),
    ...
    if content is not None or input_file is not None:
        ...
        if input_file is not None:
            if not input_file.exists():
                console.print(
                    f"[bold red]Error:[/] Input file '{input_file}' not found."
                )
                return
            ...
            try:
                ...
                console.print(
                    f"[bold red]Error:[/] Failed to read input file: {str(e)}"
                )
                return
        ...
        if output_file is not None:
            try:
                ...
                console.print(
                    f"[bold red]Error:[/] Failed to write to output file: {str(e)}"
                )
                return
...
def batch_process(
    ...
    output_dir: Optional[Path] = typer.Option(
        None, "--output-dir", "-o", help="Directory to write output files to"
    ),
    continue_on_error: bool = typer.Option(
        False, "--continue-on-error", help="Continue processing even if errors occur"
    ),
    pattern: Optional[str] = typer.Option(
        None,
        "--pattern",
        "-p",
        help="Pattern to search for (when using 'search' command)",
    ),
    replacement: Optional[str] = typer.Option(
        None,
        "--replacement",
        "-r",
        help="Replacement string (when using 'transform' command)",
    ),
):
    """Process multiple scripts with a single command."""
    if not script_list.exists():
        console.print(
            f"[bold red]Error:[/] Script list file '{script_list}' not found."
        )
        return
    ...
    console.print(
        f"[bold]Processing {len(scripts)} scripts with command '{command}':[/]"
    )
    ...
    for script_name in scripts:
        ...
        if not script_path:
            console.print(
                f"[bold red]Error:[/] Script '{script_name}' not found. Skipping."
            )
            continue
        ...
        if command == "code":
            ...
            location = (
                "Global"
                if str(get_global_scripts_dir()) in str(script_path)
                else "User"
            )
            metadata = get_script_metadata(script_path)
            ...
            update_script_metadata(
                script_path,
                {"run_count": run_count, "last_run": datetime.now().isoformat()},
            )
            ...
            if script_path.suffix == ".py":
                try:
                    ...
                    if spec is None or spec.loader is None:
                        console.print(
                            f"[bold red]Error:[/] Failed to load Python script '{script_name}'."
                        )
                        continue
                ...
                if matches:
                    ...
                    if output_dir is not None:
                        ...
                        try:
                            ...
                            console.print(
                                f"[green]Matches written to:[/] {output_file}"
                            )
                        except Exception as e:
            ...
            try:
                ...
                try:
                    ...
                    if transformed_content == content:
                        console.print(
                            f"[yellow]No changes made to '{script_name}'. Pattern did not match any content.[/]"
                        )
                        continue
                    ...
                    if output_dir is not None:
                        ...
                        output_file = (
                            output_dir
                            / f"{script_name}_transformed{script_path.suffix}"
                        )
                        try:
                            with open(output_file, "w") as f:
                                f.write(transformed_content)
                            console.print(
                                f"[green]Transformed content written to:[/] {output_file}"
                            )
                        except Exception as e:
                            error_msg = (
                                f"Failed to write transformed content to file: {str(e)}"
                            )
                            console.print(f"[bold red]Error:[/] {error_msg}")
                            if not continue_on_error:
                                return
                        ...
                        if not os.access(script_path, os.W_OK):
                            console.print(
                                f"[bold red]Error:[/] You don't have permission to edit script '{script_name}'."
                            )
                            if not continue_on_error:
                                continue
                        else:
                            ...
                            console.print(
                                f"[green]Transformed script:[/] {script_name}"
                            )
                except Exception as e:
            ...
            if (
                command not in ["code", "info", "run"]
                and (command == "search" and pattern is None)
                or (command == "transform" and (pattern is None or replacement is None))
            ):
                console.print(
                    f"[bold red]Error:[/] Command '{command}' requires additional parameters. Skipping."
                )
            else:
                console.print(
                    f"[bold red]Error:[/] Unknown command '{command}'. Skipping."
                )
...
def update_metadata(
    ...
    description: str = typer.Option(
        None, "--description", "-d", help="Description of the script"
    ),
    global_script: bool = typer.Option(
        True, "--global/--no-global", help="Include global scripts"
    ),
...
def list_templates():
    ...
    if not templates:
        ...
        console.print(
            f"Templates should be placed in {get_templates_dir()}/[type]/[template].[ext]"
        )
        return
...
def transform_script(
    ...
    pattern: str = typer.Option(
        ..., "--pattern", "-p", help="Regular expression pattern to search for"
    ),
    replacement: str = typer.Option(
        ..., "--replacement", "-r", help="Replacement string"
    ),
    output_file: Optional[Path] = typer.Option(
        None,
        "--output",
        "-o",
        help="Write transformed content to this file instead of updating the script",
    ),
    global_script: bool = typer.Option(
        True, "--global/--no-global", help="Include global scripts"
    ),
    backup: bool = typer.Option(
        True, "--backup/--no-backup", help="Create a backup before transforming"
    ),
):
    ...
    try:
        ...
        if transformed_content == content:
            console.print(
                "[yellow]Warning:[/] No changes made. Pattern did not match any content."
            )
            return
...
def search_scripts(
    ...
    regex: bool = typer.Option(
        False, "--regex", "-r", help="Treat pattern as a regular expression"
    ),
    case_sensitive: bool = typer.Option(
        False, "--case-sensitive", "-c", help="Make search case-sensitive"
    ),
    global_scripts: bool = typer.Option(
        True, "--global/--no-global", help="Include global scripts"
    ),
    output_file: Optional[Path] = typer.Option(
        None, "--output", "-o", help="Write results to this file"
    ),
    ignore_errors: bool = typer.Option(
        False, "--ignore-errors", help="Continue searching even if errors occur"
    ),
):
    ...
    if output_file is not None:
        ...
        if errors and not ignore_errors:
            console.print(
                f"[yellow]Note: {len(errors)} errors occurred during search. Use --ignore-errors to suppress these messages.[/]"
            )
...
def generate_script(
    ...
    api_key: Optional[str] = typer.Option(
        None, "--api-key", "-k", help="API key for the AI service"
    ),
    local: bool = typer.Option(
        False, "--local", "-l", help="Use local templates instead of AI"
    ),
):
    ...
    if name == "count_lines":
        try:
            ...
            console.print(
                f"[green]Using '{template_name}' template for line counting script.[/]"
            )
        except FileNotFoundError:
    ...
    if not use_local_template:
        ...
        try:
            ...
            console.print(
                "[yellow]Warning:[/] The 'requests' package is required for AI generation."
            )
            console.print(
                "Falling back to local template. Install requests with: pip install requests"
            )
            use_local_template = True
        ...
        if not use_local_template:
            ...
            if api_key is None:
                ...
                if api_key is None:
                    ...
                    console.print(
                        "Falling back to local template. Set the OPENAI_API_KEY environment variable or use --api-key."
                    )
                    use_local_template = True
...
"""
        ...
        with console.status(
            f"[bold green]Generating {language} script using {model}...[/]"
        ):
            try:
                ...
                if response.status_code != 200:
                    console.print(
                        f"[yellow]Warning:[/] API request failed with status code {response.status_code}"
                    )
                    ...
                    generated_code = response_data["choices"][0]["message"][
                        "content"
                    ].strip()
                ...
                console.print(
                    f"[yellow]Warning:[/] Failed to connect to the API: {str(e)}"
                )
                console.print("Falling back to local template.")
                use_local_template = True
            except Exception as e:
    ...
    if use_local_template and generated_code is None:
        ...
        if best_match[1] > 0:
            try:
                ...
                try:
                    ...
                    console.print(
                        f"[bold red]Error:[/] No templates found for {type.lower()}."
                    )
                    return
        else:
            # Use basic template
        ...
        if type.lower() == "python":
            generated_code = generated_code.replace(
                "Custom script for SkogCLI.", f"Script to {description}"
            )
        ...
        console.print(
            f"[green]Using '{template_name}' template for script generation.[/]"
        )
...
def import_script(
    ...
    overwrite: bool = typer.Option(
        False, "--overwrite", help="Overwrite existing script"
    ),
):
    ...
    for field in required_fields:
        if field not in export_data:
            console.print(
                f"[bold red]Error:[/] Export file is missing required field: {field}"
            )
            return
...
def import_file(
    ...
    name: Optional[str] = typer.Option(
        None, "--name", "-n", help="Name for the script (defaults to original filename)"
    ),
    global_script: bool = typer.Option(
        False, "--global", "-g", help="Install as a global script"
    ),
...
def copy_script(
    ...
    global_source: bool = typer.Option(
        True,
        "--global-source/--no-global-source",
        help="Include global scripts as source",
    ),
    global_dest: bool = typer.Option(
        False, "--global-dest", "-g", help="Create as a global script"
    ),
    edit: bool = typer.Option(
    ...
    if global_dest:
        ...
        if not os.access(global_dir.parent, os.W_OK):
            ...
            console.print(
                "Try running with sudo or use --no-global-dest for a user script."
            )
            return
        dest_dir = global_dir
