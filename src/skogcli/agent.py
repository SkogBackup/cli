...
def migrate_scripts():
    ...
    console.print(
        f"[green]Migration complete:[/] {migrated} agent scripts created in {scripts_dir}"
    )
        ...
        if "." in arg and not arg.startswith("-"):
            ...
            if len(parts) == 2:
                ...
                try:
                    if (
                        remaining_args
                        and remaining_args[0] == "send"
                        and len(remaining_args) > 1
                    ):
...
def send(
    ...
    raw: bool = typer.Option(
        False, "--raw", "-r", help="Display raw response without formatting"
    ),
    json_output: bool = typer.Option(
        False, "--json", "-j", help="Output response in JSON format"
    ),
):
    """
...
def set_agent_config(
    ...
    if key.startswith("agent."):
        ...
        console.print(
            "[bold red]Error:[/] Agent name must be provided either in the key or with --agent"
        )
        return
...
def get_agent_config(
    ...
    if value is None:
        console.print(
            f"[bold red]Error:[/] Key '{full_key}' not found in configuration."
        )
        return
...
def read_agent(
    ...
    if agent is None:
        console.print(
            "[bold red]Error:[/] Agent name must be provided either as an argument or with --agent"
        )
        raise typer.Exit(1)
    ...
    # Display the response
    if json_output:
        output = {"agent": agent, "message": message, "config": agent_config}
        print(json.dumps(output, indent=2))
    elif raw:
        print(message)
    else:
        console.print(
            Panel(Markdown(message), title=f"Agent: {agent}", border_style="blue")
        )
